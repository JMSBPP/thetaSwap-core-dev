...
\[
  \{B_T = \texttt{hook.feeConcentrationIndex()}\}
  \;\longrightarrow\;
  \{p_{\text{oracle}} = B_T / (1 - B_T)\}
\]
The oracle price derived from hook state matches the odds-ratio mapping.
\end{invariant}

\begin{invariant}{CFMM-29}{Jump-adjusted replication}
% Kontrol: prove_cfmm_feeconc_jumpAdjusted
\[
  \{B_T \text{ jumps from } b \text{ to } b'\}
  \;\longrightarrow\;
  \{\Delta\text{LP\_value} \leq \VLP(b'/(1-b')) - \VLP(b/(1-b))\}
\]
Discrete jumps in the index at liquidity events do not cause LP losses beyond the
...
\[
  ...
  \;\longrightarrow\;
  \{B_T \text{ updated} \;\land\; \text{funding settled} \;\land\; \text{P\&L realized}\}
\]
...
\[
  0 \leq B_T \leq 1
\]
always, since $B_T = 1 - A_T$ and $A_T \in [0, 1]$. This ensures
$p = B_T/(1-B_T) \in [0, \infty)$ and reserves remain well-defined.
\end{invariant}
